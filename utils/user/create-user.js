...
const fs = require('fs');
const {createFakeEmail, createFakeUsername, createFakeDOB, createFakeAvatar} = require('./user-helpers');
...
for(let i = 1; i <= numberOfUsers; i++){
  ...
  let avatar = {};
  const fakeAvatars = createFakeAvatar();
  avatar = { id: i, ...fakeAvatars };
...
const avatarsData = JSON.stringify(avatarsArray, null, 2);
fs.writeFile('./data/users/avatars.json', avatarsData, ()=>{console.log('wrote to avatars.json')});